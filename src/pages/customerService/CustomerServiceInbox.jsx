...
const UserAvatar = ({ name, className = "w-12 h-12" }) => {
    ...
    return (
        <div
            className={`${className} rounded-xl bg-gray-50 border border-[#DCDCDC] text-gray-800 flex items-center justify-center font-black flex-shrink-0 shadow-sm`}
        >
            {initial}
        </div>
    );
...
const ChatContact = ({ name, lastMessage, time, unread, onClick, isSelected }) => {
    const needsAgent = lastMessage?.toLowerCase().includes("talk to a real person");

    return (
        <div
            onClick={onClick}
            className={`flex items-start justify-between p-4 cursor-pointer transition-all rounded-2xl mb-1.5 border ${
                isSelected
            ...
            <div className="flex items-start space-x-3 min-w-0 flex-1">
                <UserAvatar
                    ...
                    className={`w-10 h-10 text-xs flex-shrink-0 ${
                        needsAgent && !isSelected ? "bg-orange-100 border-orange-200" : ""
                    }`}
                />
                <div className="flex flex-col gap-1 min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        {needsAgent && (
                            <span className="bg-orange-500 text-white text-[7px] font-black uppercase px-1.5 py-0.5 rounded-md tracking-tighter">
                                Needs Agent
                            </span>
                        )}
                    </div>
                    <span
                        className={`text-[11px] truncate max-w-[150px] font-medium italic ${
                            needsAgent ? "text-orange-600 font-bold" : "text-gray-500"
                        }`}
                    >
            ...
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0 ml-2">
                <span
                    className={`text-[9px] font-black uppercase tracking-tighter ${
                        needsAgent ? "text-orange-400" : "text-gray-300"
                    }`}
                >
                    {time}
                </span>
...
const CustomerServiceInbox = () => {
    ...
    const isMessageFromAdmin = useCallback((msg, contactUserId) => {
        const adminTypes = ["admin", "subadmin", "sub_admin", "artist", "staff", "customer_service"];
    ...
    const formatMsg = useCallback(
        (msg, contactUserId) => {
            ...
            return {
                ...
                video: msg.video ? resolveImage(msg.video) : null,
    ...
    const loadConversations = useCallback(async () => {
        try {
            ...
            const formatted = Array.from(seenUsers.values()).map((conv) => {
                const u = conv.user || {};
                const name = u.first_name
                    ? `${u.first_name} ${u.last_name || ""}`.trim()
                    : conv.sender_name || conv.customer_name || u.name || "Unknown";
                const lastMsg = (conv.last_message || "No messages yet").replace(/^\[DESIGN\]\s*/i, "");
                return {
                    ...
                    name,
                    lastMessage: lastMsg,
                    time: formatTime(conv.last_at || conv.updated_at || conv.created_at),
    ...
    const loadMessages = useCallback(
        async (contact, isPolling = false) => {
            if (!contact?.userId) return;
            try {
                if (!isPolling) setLoadingMessages(true);
                const data = await fetchAdminUserMessages(contact.userId);
    ...
    useEffect(() => {
        if (!selectedContact) return;
        const interval = setInterval(() => loadMessages(selectedContact, true), 3000);
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            const res = await sendAdminMessage(null, selectedContact.userId, file);
            const real = res.message || res;
            setMessages((prev) =>
                prev.map((m) =>
                    m.id === optimistic.id ? formatMsg(real, selectedContact.userId) : m
                )
            );
            loadConversations();
        } catch (err) {
    ...
    const handleSend = async () => {
        ...
        try {
            const res = await sendAdminMessage(content, selectedContact.userId, null);
            const real = res.message || res;
            setMessages((prev) =>
                prev.map((m) =>
                    m.id === optimistic.id ? formatMsg(real, selectedContact.userId) : m
            ...
            if (res.success) {
                const update = (c) =>
                    c.userId === selectedContact.userId
                        ...
                        : c;
                setSelectedContact((prev) => update(prev));
    ...
    const renderMsg = (msg) => (
        <div key={msg.id} className={`flex ${msg.isAdmin ? "justify-end" : "justify-start"} mb-5`}>
            <div className={`flex items-end gap-3 max-w-[75%] ${msg.isAdmin ? "flex-row-reverse" : ""}`}>
                {!msg.isAdmin && (
                    <UserAvatar name={selectedContact?.name} className="w-9 h-9 text-xs" />
                )}
                        ...
                        {msg.video && (
                            <div className="mb-3">
                                <video
                                    src={msg.video}
                                    controls
                                    className="max-h-56 w-full rounded-xl border border-gray-200"
                                />
                            </div>
                        )}
                        {msg.content && (
                            <p className="text-[14px] whitespace-pre-wrap leading-relaxed tracking-tight">
                                {msg.content}
                            </p>
                        )}
                        <div
                            className={`text-[9px] mt-2 text-right font-black uppercase tracking-[0.1em] ${
                                msg.isAdmin ? "text-yellow-900/60" : "text-gray-400"
                            }`}
                        >
                            {msg.pending ? (
                                <SendingDots />
    ...
    return (
        <div className="lg:rounded-3xl lg:my-5 lg:mr-5 lg:ml-1 h-screen lg:h-[calc(100vh-2.5rem)] flex flex-col lg:flex-row gap-0 lg:gap-4 overflow-hidden bg-[#F1F3F7] lg:bg-transparent">
            ...
            {/* ── Sidebar ── */}
            <div
                className={`flex flex-col h-full w-full lg:w-[360px] flex-shrink-0 transition-all duration-300 ${
            ...
            >
                <div className="mb-4 lg:mb-6 p-5 lg:p-0 bg-white lg:bg-transparent border-b lg:border-none border-gray-200 lg:pt-0 pt-20 lg:block flex flex-col">
                    <h1 className="text-xl lg:text-2xl font-black text-gray-900 mb-1 italic uppercase tracking-tighter">
                        Message Center
                    </h1>
                    <p className="text-[10px] lg:text-xs font-bold text-gray-400 uppercase tracking-widest px-0.5">
            ...
            {/* ── Chat area ── */}
            <div
                className={`flex-1 bg-white lg:border lg:border-[#DCDCDC] lg:rounded-[40px] flex flex-col overflow-hidden shadow-sm transition-all duration-300 ${
                ...
                {selectedContact ? (
                    <>
                        ...
                        <div className="flex items-center gap-3 lg:gap-4 p-4 lg:p-7 border-b border-[#DCDCDC]/50 bg-white/80 backdrop-blur-md sticky top-0 z-20 pt-16 lg:pt-7">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all ${
                                    selectedContact.user?.is_bot_active
                        ...
                        <div
                            ...
                            className="flex-1 p-5 lg:p-10 overflow-y-auto bg-gray-50/30 flex flex-col custom-scrollbar"
                        >
                            {loadingMessages ? (
                                <div className="flex justify-center py-10">
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-gray-300 gap-4 opacity-40">
                                    <p className="font-black uppercase tracking-[0.2em] italic text-xs">
                                        No messages yet.
                                    </p>
                                </div>
                            ) : (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-gray-300 gap-8 bg-gray-50/20">
                        ...
                        <div className="text-center">
                            <p className="font-black uppercase tracking-[0.3em] italic text-sm mb-1 opacity-40">
