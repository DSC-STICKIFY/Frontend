...
const GraphicServices = () => {
    ...
    // Auto-refresh every 60s with scroll preservation
    useEffect(() => {
    ...
    // Package details fallback (structured description or defaults)
    const getPackageDetails = (tierName, subType = "") => {
        ...
        } else if (name.includes("standar d")) {
            inclusions = [
                "60pcs 2x2 Stickers",
                "FB & Youtube Cover Design",
                "Profile Design",
    ...
    const { businessLogo, motoVlog } = useMemo(() => {
        ...
        graphicProducts.forEach(p => {
            // ✅ Use product's own promo and discounted price – no hardcoded calculation
...
// ✅ ServiceCard – uses product's own promo and discounted price
