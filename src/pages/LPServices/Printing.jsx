...
import bgPrinting from "../../assets/LPGraphicServices.png"; // Fallback to Graphic Services banner
...
const PrintingSection = ({ title, items, onSelect, loading, filterDropdown }) => (
  <div className="my-10 px-2">
    ...
    {loading ? (
      ...
      <div className="mt-5 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((item) => (
          <PrintingItem key={item.id} item={item} onSelect={onSelect} />
        ))}
      </div>
    )}
  </div>
      ...
      {!isLoading &&
        ...
        largeFormat.length === 0 && (
          <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
            <p className="text-gray-500 text-xl font-bold uppercase italic tracking-widest">No printing services available yet.</p>
          </div>
        )}

      {/* Custom Inquiry CTA */}
      <div className="flex flex-col md:flex-row gap-5 my-20 justify-center items-center w-full px-8 bg-[#FDE31E]/10 py-12 rounded-[40px] border border-[#FDE31E]/20">
        <p className="text-xl font-black text-[#0B132A] text-center md:text-left uppercase italic tracking-tight">
          Need a custom print size or volume discount?
        </p>
        <button className="bg-[#0B132A] text-white px-10 py-4 rounded-2xl hover:bg-gray-800 transition font-black uppercase italic tracking-widest text-sm shadow-xl active:scale-95">
          Request a Quote
        </button>
      </div>
...
const PrintingItem = ({ item, onSelect }) => {
  ...
  return (
    <div className="group">
      <div
        className="relative rounded-2xl overflow-hidden cursor-pointer shadow-sm group-hover:shadow-xl group-hover:-translate-y-1 transition-all duration-300"
      ...
      <div className="font-semibold mt-3 px-1">
