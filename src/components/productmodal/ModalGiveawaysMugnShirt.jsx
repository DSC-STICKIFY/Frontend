...
const ModalGiveawaysMugnShirt = ({ giveaways, onClose }) => {
  ...
  const { isShirt, isMug } = useMemo(() => {
    ...
    return { isShirt: combined.includes("shirt"), isMug: combined.includes("mug") };
  }, [giveaways]);
  ...
  useEffect(() => {
    if (!isCustomizableProduct && quantity > stockCount) setQuantity(Math.max(1, stockCount));
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white">
              <div className="px-10 pb-12 space-y-8 mt-1">
                <div className="space-y-2">
                  <p className="text-[9px] font-black uppercase tracking-[0.3em] text-yellow-500">{giveaways.type || "Apparel"}</p>
                  <h2 className="text-[34px] font-black text-gray-900 tracking-tight leading-none">{giveaways.category || giveaways.title || "Custom Item"}</h2>
        ...
        {showCustomizationRequest && (
          <ModalRequestCustomization
            product={{
              ...
              title: giveaways.category || giveaways.title || "Custom Item",
          ...
          <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-y-auto md:overflow-hidden">
            <div className="w-full md:w-1/2 flex flex-col border-r border-gray-100 md:overflow-y-auto custom-scrollbar bg-gray-50/30">
              <div className="p-5 sm:p-8 flex flex-col gap-4">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight leading-tight">{giveaways.category || giveaways.title}</h2>
                ...
                <div className="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm overflow-x-auto">
                  <h4 className="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Bulk Pricing Table</h4>
                  <table className="w-full text-xs min-w-[300px]">
                    <thead>
                      <tr className="border-b border-gray-50"><th className="py-2 text-left font-black text-gray-400 uppercase tracking-tighter text-[9px]">Quantity Range</th><th className="py-2 text-right font-black text-gray-400 uppercase tracking-tighter text-[9px]">Price Per Piece</th></tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {options.map(tier => {
                        ...
                        return (
                          <tr key={`${selectedSubtype}-${tier}`} onClick={() => { if (!isOutOfStockForTier) setQuantity(tier); }} className={`group transition-all duration-150 ${isOutOfStockForTier ? 'opacity-50 cursor-not-allowed pointer-events-none bg-gray-50/10' : isActiveTier ? 'bg-yellow-50/60 cursor-pointer' : 'hover:bg-gray-50 hover:translate-x-0.5 cursor-pointer'}`}>
                            <td className="py-2.5 sm:py-3 px-2"><div className="flex items-center gap-2"><div className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border-2 flex items-center justify-center transition-all duration-150 ${isActiveTier ? "border-[#FFE100]" : "border-gray-200"}`}>{isActiveTier && <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-[#FFE100] animate-[scale-in_0.15s_ease-out]" />}</div><span className={`font-bold text-[11px] sm:text-xs ${isActiveTier ? 'text-gray-900' : 'text-gray-500'}`}>{tier} {tier === 1 ? "piece" : "pieces"}</span></div></td>
                            <td className="py-2.5 sm:py-3 text-right pr-2">{tierHasDiscount ? (<div className="flex flex-col items-end"><span className="text-[8px] sm:text-[9px] line-through text-gray-400">₱{formatPrice(rawTierPrice)}</span><span className={`font-black italic text-[11px] sm:text-xs ${isActiveTier ? 'text-gray-900' : 'text-gray-500'}`}>₱{formatPrice(discTierPrice)}</span></div>) : (<span className={`font-black italic text-[11px] sm:text-xs ${isActiveTier ? 'text-gray-900' : 'text-gray-500'}`}>₱{formatPrice(rawTierPrice)}</span>)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-gray-400 rounded-xl text-xl font-bold transition-all duration-150 hover:bg-white hover:shadow-sm hover:text-gray-800 hover:scale-110 active:scale-95">−</button>
                    <input type="number" min="1" value={quantity} onChange={(e) => { const val = Math.max(1, parseInt(e.target.value) || 1); setQuantity(!isCustomizableProduct ? Math.min(stockCount, val) : val); }} className="w-16 px-2 py-1.5 text-center font-black border border-gray-200 rounded-xl focus:border-[#FFE100] focus:outline-none text-sm" />
                    <button onClick={() => setQuantity(q => !isCustomizableProduct ? Math.min(stockCount, q + 1) : q + 1)} disabled={!isCustomizableProduct && quantity >= stockCount} className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center text-gray-400 rounded-xl text-xl font-bold transition-all duration-150 hover:bg-white hover:shadow-sm hover:text-gray-800 hover:scale-110 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed">+</button>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex-1 space-y-6">
                ...
                {((!isCustomMode && hasDbSizes) || hasDbDesigns) && (
                  <div className="bg-gray-50/50 p-4 sm:p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    ...
                    {hasDbDesigns && (
                      <div><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">1. Choose Design</h3>
                        <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => setSelectedDesign(null)} className={`relative flex items-center gap-3 p-3 rounded-xl border-2 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm ${selectedDesign === null ? 'border-yellow-400 bg-yellow-50/30' : 'border-gray-100 bg-white hover:border-yellow-200'}`}>
                            ...
                            <div className="text-left min-w-0"><span className="text-[10px] font-bold text-gray-800 block truncate leading-tight">Standard Design</span><span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Default</span></div>
                          </button>
                          {giveaways.designs.map((design) => {
                            ...
                            return (
                              <button key={design.id} onClick={() => setSelectedDesign(design)} className={`relative flex items-center gap-3 p-3 rounded-xl border-2 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm ${isSelected ? 'border-yellow-400 bg-yellow-50/30' : 'border-gray-100 bg-white hover:border-yellow-200'}`}>
                                ...
                                <div className="text-left min-w-0"><span className="text-[10px] font-bold text-gray-800 block truncate leading-tight">{design.design_name}</span>{parseFloat(design.additional_price) > 0 ? (<span className="text-[8px] font-black text-yellow-600">+ ₱{parseFloat(design.additional_price).toFixed(2)}</span>) : (<span className="text-[8px] font-black text-gray-400 uppercase tracking-tighter">Free</span>)}</div>
                              </button>
                            );
                          })}
                        </div>
                    ...
                    {!isCustomMode && hasDbSizes && (
                      <div><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{hasDbDesigns ? "2." : "1."} Size Option</h3>
                          ...
                          {giveaways.sizes.map((size) => {
                            ...
                            return (
                              <button key={size.id} onClick={() => setSelectedSizeObj(size)} className={`px-4 py-2 rounded-xl border-2 text-xs font-bold transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm flex flex-col items-center justify-center gap-0.5 ${isSelected ? 'border-yellow-400 bg-yellow-50 text-gray-900' : 'border-gray-100 bg-white text-gray-600 hover:border-yellow-200'}`}>
                                <span>{size.size_name}</span>{parseFloat(size.additional_price) > 0 && <span className="text-[9px] font-black text-yellow-600">+₱{parseFloat(size.additional_price)}</span>}
                              </button>
                            );
                          })}
                        </div>
                ...
                <div><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 italic">Order Summary</h3><div className="space-y-6"><div className="flex justify-between text-xs sm:text-sm pt-2 px-1"><span className="text-gray-500">Tier Unit Price</span>{hasDiscount ? (<div className="flex flex-col items-end"><span className="text-[9px] sm:text-[10px] line-through text-gray-400">₱ {formatPrice(adjustedUnitPrice)}</span><span className="font-bold text-gray-900">₱ {formatPrice(unitPrice)}</span></div>) : (<span className="font-bold text-gray-900">₱ {formatPrice(unitPrice)}</span>)}</div><div className="flex justify-between text-xs sm:text-sm px-1"><span className="text-gray-500">Total Pieces</span><span className="font-bold text-gray-900">{totalQuantity} Piece(s)</span></div></div></div>
