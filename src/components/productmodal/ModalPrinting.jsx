...
const ModalPrinting = ({ product, onClose }) => {
  ...
  useEffect(() => {
    ...
    const combined = `${t} ${d}`;
    if (isPrint) { if (combined.includes("color")) setSelectedSubtype("Colored"); else setSelectedSubtype("Black"); }
    else if (isPhotocopy) { if (combined.includes("back") || combined.includes("both")) setSelectedSubtype("Front & Back"); else setSelectedSubtype("Front Only"); }
  }, [product.id, product.product_id, isPrint, isPhotocopy]);

  const rawPrice = useMemo(() => {
    if (isPrint) return selectedSubtype === "Colored" ? 10 : 5;
    if (isPhotocopy) return selectedSubtype === "Front & Back" ? 10 : 5;
  ...
  useEffect(() => { if (!isCustomizableProduct && quantity > stockCount) setQuantity(Math.max(1, stockCount)); }, [isOutOfStock, stockCount, isCustomizableProduct, quantity]);
  ...
  useEffect(() => { setSubtotal(discountedPrice * quantity); }, [quantity, discountedPrice]);
  ...
  const buildPayload = () => ({
    ...
    quantity, category, type: product.type || "Printing", subtotal, initialPaymentMethod: paymentMethod, customMode: isCustomMode ? "custom" : "standard", designImage: isCustomMode ? (uploadedImage?.preview || null) : null, timestamp: Date.now()
  });
  ...
  const handleBuyNow = async () => {
    ...
    try {
      if (isCustomMode && uploadedImage?.preview) { await sendCustomerMessage(`[DESIGN] Interested in ${title}. Qty: ${quantity}. Subtotal: ₱${formatPrice(subtotal)}.`, null, product.id || product.product_id); }
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white">
              <div className="px-10 pb-12 space-y-8 mt-1">
                ...
                <div className="grid grid-cols-3 gap-4">{[
                  { icon: "🖨️", label: "Any Size", sub: "Custom dimensions" },
          ...
          <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-y-auto md:overflow-hidden">
            ...
            <div ref={rightPanelRef} className="w-full md:w-1/2 p-5 sm:p-8 md:overflow-y-auto custom-scrollbar bg-white flex flex-col">
              <div className="flex-1 space-y-6">
                ...
                <div><h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 italic">Configuration Summary</h3><div className="space-y-6"><div className="flex justify-between text-xs sm:text-sm pt-2 px-1"><span className="text-gray-500">Unit Price</span><span className="font-bold text-gray-900">₱ {formatPrice(discountedPrice)}</span></div><div className="flex justify-between text-xs sm:text-sm px-1"><span className="text-gray-500">Quantity</span><span className="font-bold text-gray-900">{quantity} Units</span></div></div></div>
                ...
                <div ref={subtotalRef} className="bg-gray-900 rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 text-white shadow-2xl shadow-gray-200 relative overflow-hidden group mt-4 transition-all duration-300 hover:shadow-gray-300"><div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-white/5 rounded-full -mr-12 -mt-12 sm:-mr-16 sm:-mt-16 transition-transform duration-300 group-hover:scale-125" /><p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">Estimated Total</p><div className="flex items-baseline gap-1"><span className="text-lg sm:text-xl font-bold text-yellow-400">₱</span><span className="text-4xl sm:text-5xl font-black tracking-tighter italic">{formatPrice(subtotal)}</span></div></div>
              </div>
              <div className="mt-8 space-y-3 sm:space-y-4">
                <button onClick={handleBuyNow} disabled={isOutOfStock || isSubmitting || subtotal <= 0 || !paymentMethod || (currentUser && !isVerified) || (isCustomMode && !uploadedImage?.preview)} className={`group/checkout w-full py-5 sm:py-6 rounded-[20px] sm:rounded-[24px] font-black uppercase tracking-widest text-xs sm:text-sm transition-all duration-200 active:scale-[0.97] flex items-center justify-center gap-3 ${(isOutOfStock || isSubmitting || subtotal <= 0 || !paymentMethod || (currentUser && !isVerified) || (isCustomMode && !uploadedImage?.preview)) ? "bg-gray-100 text-gray-300 cursor-not-allowed shadow-none" : "bg-[#FFE100] text-black shadow-xl shadow-yellow-200/60 hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-yellow-300/50"}`}>{isOutOfStock ? "Out of Stock" : (isCustomMode && !uploadedImage?.preview) ? "Upload Design to Proceed" : (currentUser && !isVerified ? "Verification Required" : isSubmitting ? "Processing..." : "Proceed to Checkout")}</button>
